...
const _squigglyWorkerCode = `
  ...
  function _drawGameProgress(style) {
    ...
    if (style === "inchworm") {
      const bodyEnd = totalProgressPx - headR * 0.5;
      const humpLen = 9, humpH = playing ? 3.2 : 1;
      ctx.beginPath();
      let first = true;
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(totalProgressPx, cy, headR * 0.9, 0, Math.PI * 2);
      ctx.fillStyle = overlay ? "#fff" : waveColor;
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(totalProgressPx - headR * 0.3, cy - headR * 0.7);
      ctx.lineTo(totalProgressPx - headR * 0.5, cy - headR * 1.3);
      ctx.moveTo(totalProgressPx + headR * 0.3, cy - headR * 0.7);
      ctx.lineTo(totalProgressPx + headR * 0.5, cy - headR * 1.3);
      ctx.strokeStyle = overlay ? "#fff" : waveColor;
      ctx.lineWidth = 1;
    ...
    if (style === "ant") {
      const bodyEnd = totalProgressPx - headR * 0.6;
      const segSpacing = 6, segR = 2.2;
      let si = 0;
      for (let x = bodyEnd; x > leftInset; x -= segSpacing) {
        ctx.beginPath();
        ctx.arc(x, cy, segR, 0, Math.PI * 2);
        ctx.fillStyle = overlay ? "#fff" : waveColor;
        ctx.globalAlpha = si % 2 === 0 ? 1 : 0.6;
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(x, cy - segR);
        ctx.lineTo(x - 2, cy - segR - 3);
        ctx.moveTo(x, cy + segR);
        ctx.lineTo(x - 2, cy + segR + 3);
        ctx.strokeStyle = overlay ? "rgba(255,255,255,0.6)" : _shade(waveColor, 0.6);
        ctx.lineWidth = 0.8;
        ctx.stroke();
        ctx.globalAlpha = 1;
        si++;
      ...
      if (crumbX > totalProgressPx + headR) {
        ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(totalProgressPx, cy, headR * 0.85, 0, Math.PI * 2);
      ctx.fillStyle = overlay ? "#fff" : waveColor;
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(totalProgressPx + headR * 0.4, cy - headR * 0.5);
      ctx.lineTo(totalProgressPx + headR * 1.1, cy - headR * 1.1);
      ctx.moveTo(totalProgressPx + headR * 0.4, cy + headR * 0.1);
      ctx.lineTo(totalProgressPx + headR * 1.1, cy - headR * 0.4);
      ctx.strokeStyle = overlay ? "#fff" : waveColor;
      ctx.lineWidth = 1;
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(totalProgressPx + headR * 0.15, cy - headR * 0.3, headR * 0.15, 0, Math.PI * 2);
      ctx.fillStyle = overlay ? "#333" : _shade(waveColor, 0.25);
      ctx.fill();
      return;
    }
  ...
  function _draw() {
    ...
    if (thumbStyle === "pacman" || thumbStyle === "snake" || thumbStyle === "inchworm" || thumbStyle === "ant" || thumbStyle === "firefly") {
  ...
  _drawGameProgress(ctx, style) {
    ...
    if (style === "ant") {
      ...
      for (let x = bodyEnd; x > leftInset; x -= segSpacing) {
        ...
        ctx.moveTo(x, cy + segR);
        ctx.lineTo(x - 2, cy + segR + 3);
        ctx.strokeStyle = overlay ? "rgba(255,255,255,0.6)" : this._shade(waveColor, 0.6);
        ctx.lineWidth = 0.8;
        ctx.stroke();
        ctx.globalAlpha = 1;
        si++;
 ...
 * "inchworm" = looping measuring-worm body with eyes and antennae
      ...
      <div class="section-panel">
        ...
        <div class="settings-row settings-row--wrap">
          ...
          <div class="settings-btn-group">
            ...
            <button type="button" class="thumbstyle-btn settings-toggle-btn${(state.settings.squigglyThumbStyle || "circle") === "inchworm" ? " active" : ""}" data-style="inchworm">
              🐛 Inchworm
            </button>
      ...
      <div class="help-sections">
        <div class="section-panel" id="help-updates-section">
          ...
          <div class="help-item">
            ...
            <div class="help-item-body" style="padding-left: 15px;">
              <ul style="padding-left: 10px;">
                <li><strong>Seven Thumb Styles:</strong> Choose your progress bar scrubber head style in <strong>Settings → Scrubber Head</strong>.</li>
                ...
                <li><strong>Inchworm</strong>: A looping measuring-worm arches its way along the bar with little antennae and eyes.</li>
