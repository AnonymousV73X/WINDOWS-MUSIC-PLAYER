  ...
  function _drawGameProgress(style) {
    ...
    if (style === "pacman") {
      ...
      if (pmWaveEndPx > leftInset) {
        const pmWaveProgressPx = W * (progress > matchedWaveEndpoint ? progress : _lerp(minWaveEndpoint, matchedWaveEndpoint, _lerpInv(0, matchedWaveEndpoint, progress)));
        ...
        const pmComputeAmp = (x) => {
          ...
          const headCoeff = _lerpInvSat(pmWaveProgressPx + length / 2, pmWaveProgressPx - length / 2, x);
          const leftCoeff = _lerpInvSat(leftInset, leftInset + pmEdgeTaper, x);
          const rightCoeff = _lerpInvSat(pmWaveEndPx, pmWaveEndPx - pmEdgeTaper, x);
