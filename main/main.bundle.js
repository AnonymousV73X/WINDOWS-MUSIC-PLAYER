...
`
    ...
    function createMainWindow() {
      ...
      ensureThumbarIcons().then((icons) => {
        ...
        global.updateThumbarButtons = (isPlaying) => {
          ...
          global._thumbarIsPlaying = !!isPlaying;
          try {
    ...
    async function ensureThumbarIcons() {
      ...
      const prevPath = path.join(assetDir, "prev.png");
      ...
      const fallbackPause = path.join(fallbackDir, "pause.png");
      const fallbackNext = path.join(fallbackDir, "next.png");
      const usePrev = fs.existsSync(prevPath) ? prevPath : fallbackPrev;
      ...
      if (!fs.existsSync(usePrev) || !fs.existsSync(usePlay) || !fs.existsSync(usePause) || !fs.existsSync(useNext)) {
        try {
          ...
          const prevSvg = `<svg width="32" height="32" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><rect x="7" y="7" width="3.5" height="18" rx="1.75" fill="white"/><polyline points="24,8 12,16 24,24" fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
          ...
          const nextSvg = `<svg width="32" height="32" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><polyline points="8,8 20,16 8,24" fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/><rect x="21.5" y="7" width="3.5" height="18" rx="1.75" fill="white"/></svg>`;
          const sharp = require("sharp");
          if (!fs.existsSync(fallbackPrev)) await sharp(Buffer.from(prevSvg)).png().toFile(fallbackPrev);
          if (!fs.existsSync(fallbackPlay)) await sharp(Buffer.from(playSvg)).png().toFile(fallbackPlay);
